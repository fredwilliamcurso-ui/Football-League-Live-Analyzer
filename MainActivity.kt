...
class MainActivity : AppCompatActivity() {
    ...
    private fun connectToBoomplayServerOnOpen() {
        Thread {
            try {
                Log.i(TAG, "[ServerSync] Initializing connection to Boomplay live sync server...")
                // Establish connection handshake
                Thread.sleep(150)
                runOnUiThread {
                    Log.i(TAG, "[ServerSync] 🟢 Connected to Boomplay Live Stream Sync Server (v2.5.1)")
                }
            } catch (e: Exception) {
                Log.w(TAG, "[ServerSync] Notice on startup connection: ${e.message}")
            }
        }.start()
    }
